...
export const updateProductsSchema = z.object({
  body: z.object({
    products: z.array(productUpdateSchema).min(1, { message: 'At least one product is required' }),
  }),
});
...
export const deleteProductsSchema = z.object({
  body: z.object({
    ids: z.array(z.string().uuid({ message: 'Valid product ID is required' }))
...
export const getInStockSchema = z.object({
  query: z.object({
    minStock: z.string()
      ...
      .refine((val) => !isNaN(val) && val >= 0, { message: 'minStock must be a non-negative integer' }),
  }),
}); 